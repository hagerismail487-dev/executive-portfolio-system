...
export default function RealEstateDecisionIntelligencePresentation() {
    ...
    <main className="min-h-screen bg-[#020812] text-white">
      ...
      <div className="flex min-h-screen flex-col">
        ...
        <header className="flex items-center justify-between border-b border-[#14375c] bg-[#041120]/95 px-4 py-3 backdrop-blur sm:px-6">
          ...
          <Link
            href="/case-studies/real-estate-decision-intelligence"
        ...
        {isFullscreen && (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#00050b] p-4 sm:p-6">

            <button
              onClick={() => setIsFullscreen(false)}
              aria-label="Close presentation mode"
              className="absolute right-4 top-4 z-10 inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#31506f] bg-[#07192b] text-[#9bb5cd] transition hover:border-[#5b84aa] hover:text-white"
            >
              <X size={17} />
            </button>
            ...
            <button
              onClick={previousSlide}
              aria-label="Previous dashboard"
              className="absolute left-3 top-1/2 z-10 hidden h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full border border-[#31506f] bg-[#07192b]/90 text-[#79c2ff] transition hover:border-[#5b84aa] hover:text-white sm:flex"
            >
              <ArrowLeft size={20} />
            </button>
            ...
            <button
              onClick={nextSlide}
              aria-label="Next dashboard"
              className="absolute right-3 top-1/2 z-10 flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full border border-[#31506f] bg-[#07192b]/90 text-[#79c2ff] transition hover:border-[#5b84aa] hover:text-white"
            >
              <ArrowRight size={20} />
            </button>

            <div className="absolute bottom-5 left-1/2 -translate-x-1/2 rounded-full border border-[#31506f] bg-[#07192b]/90 px-4 py-2 text-[10px] text-[#b9cce0] backdrop-blur">
              {String(activeSlide + 1).padStart(2, "0")}
              {" / "}
              {String(project.dashboards.length).padStart(2, "0")}
            </div>
